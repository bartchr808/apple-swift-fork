...
private:
  ...
  void addEnumToStruct(SILBasicBlock *originalBB, SILBasicBlock *nextBB,
    ...
    auto bbId = "bb" + std::to_string(nextBB->getDebugID());
    auto *linearMapStruct = getLinearMapStruct(nextBB);
    ...
    branchingTraceEnumCases.insert({{nextBB, originalBB}, enumEltDecl});
  }
  ...
  EnumDecl *createBranchingTraceDecl(SILBasicBlock *originalBB,
    ...
    case AutoDiffAssociatedFunctionKind::VJP: {
      for (auto *predBB : originalBB->getPredecessorBlocks()) {
        addEnumToStruct(originalBB, predBB, branchingTraceDecl);
      }
      break;
    }
    }
...
public:
  ...
  // TODO: create a better naming since the argument order is flipped for
  // JVP and VJP.
...
void LinearMapInfo::populateLinearMapStructDeclarationFields(
  ...
  for (auto &origBB : *original) {
    ...
    auto jvpSkipBlock = origExit == &origBB &&
        kind == swift::AutoDiffAssociatedFunctionKind::JVP;
    auto vjpSkipBlock = origBB.isEntry() &&
        kind == swift::AutoDiffAssociatedFunctionKind::VJP;
    if (jvpSkipBlock || vjpSkipBlock)
      continue;
    VarDecl *traceEnumField;
    switch (kind) {
    case swift::AutoDiffAssociatedFunctionKind::JVP:
      traceEnumField =
          addVarDecl(linearMapStruct, astCtx.getIdentifier("successor").str(),
...
private:
  ...
  /// tangent values.
  DenseMap<std::pair<SILBasicBlock *, SILValue>, AdjointValue> tangentValueMap;
  ...
  /// Need to add an additional argument which is the pointer to the payload.
  ...
  void addBranchInstToTrampolineBlock(SILLocation loc,
    ...
    auto succStructLoweredTy = context.getTypeConverter().getLoweredType(
        succStruct->getDeclaredInterfaceType(), ResilienceExpansion::Minimal);
...
//        enumLoweredTy.getEnumElementType(enumEltDecl, getModule()));
    ...
    trampBuilder.createStore(loc, succEnum, succField,
  ...
  SILValue getTangentProjection(SILValue originalProjection) {
    auto diffBuilder = getDifferentialBuilder();

    // Handle `struct_element_addr`.
    if (auto *seai = dyn_cast<StructElementAddrInst>(originalProjection)) {
      auto adjSource = getTangentBuffer(seai->getOperand());
    ...
    if (auto *teai = dyn_cast<TupleElementAddrInst>(originalProjection)) {
      ...
      auto adjSource = getTangentBuffer(source);
    ...
    if (auto *bai = dyn_cast<BeginAccessInst>(originalProjection)) {
      auto adjBase = getTangentBuffer(bai->getOperand());
      if (errorOccurred)
        return (bufferMap[originalProjection] = SILValue());
  ...
  SILValue &getTangentBuffer(SILValue originalBuffer) {
    ...
    if (auto *seai = dyn_cast<StructElementAddrInst>(originalBuffer)) {
      if (seai->getField()->getAttrs().hasAttribute<NoDerivativeAttr>()) {
        ...
        errorOccurred = true;
        return (bufferMap[originalBuffer] = SILValue());
      }
    }

    // If the original buffer is a projection, return a corresponding projection
    // into the tangent buffer.
    if (auto tanProj = getTangentProjection(originalBuffer)) {
      return (bufferMap[originalBuffer] = tanProj);
    }
  ...
  AdjointValue getTangentValue(SILBasicBlock *origBB, SILValue originalValue) {
  ...
  void setTangentValue(SILBasicBlock *origBB, SILValue originalValue,
    ...
    assert(newTangentValue.getType() ==
           getRemappedTangentType(originalValue->getType()));
    auto insertion =
        tangentValueMap.try_emplace({origBB, originalValue}, newTangentValue);
    ...
    for (auto origArg : ai->getArguments()) {
      ...
      if (origArg->getType().isObject()) {
        ...
        if (auto *tupleExtract =
          ...
          auto tangentTuple = materializeTangent(getTangentValue(bb, ai), loc);
          tanParam = diffBuilder.emitTupleExtract(
              loc, tangentTuple, tupleExtract->getFieldNo());
        }
        // Otherwise, materialize tangent value of `ai`.
        else {
          tanParam = materializeTangent(getTangentValue(bb, origArg), loc);
        }
      } else {
  ...
  void emitTangentForStructExtractInst(StructExtractInst *sei) {
    ...
    auto tanOperand =
        materializeTangent(getTangentValue(bb, sei->getOperand()), sei->getLoc());
  ...
  void emitTangentForLoadInst(LoadInst *li) {
    ...
    auto tanValSrc = getTangentBuffer(li->getOperand());
  ...
  void emitTangentForStoreInst(StoreInst *si) {
    ...
    auto &diffBuilder = getDifferentialBuilder();
    auto tanValSrc = materializeTangent(getTangentValue(bb, si->getSrc()),
                                        si->getLoc());
  ...
  void emitTangentForStructInst(StructInst *si) {
    ...
    for (auto elem : si->getElements())
      tangentElements.push_back(getTangentValue(bb, elem).getConcreteValue());
  ...
  void emitTangentForTupleInst(TupleInst *ti) {
    ...
    for (auto elem : ti->getElements()) {
      tangentTupleElements.push_back(
          materializeTangent(getTangentValue(bb, elem), loc));
    }
...
//        }
  ...
  void emitTangentForTupleElementAddrInst(TupleElementAddrInst *teai) {
    ...
    bufferMap.try_emplace(teai, tanBuffer);
  }
  ...
  void emitTangentForTupleExtractInst(TupleExtractInst *teai) {
    ...
    auto tanSource = materializeTangent(
        getTangentValue(bb, teai->getOperand()), loc);
    SILValue tanBuffer;
    // If the tangent buffer of the source does not have a tuple type, then
    // it must represent a "single element tuple type". Use it directly.
    if (!tanSource->getType().is<TupleType>()) {
  ...
  void visitInstructionsInBlock(SILBasicBlock *bb) {
    ...
    if (bb != original->getEntryBlock()) {
      ...
      auto loweredStructDeclTy =
          context.getTypeConverter().getLoweredType(
...
//    }
    ...
    // Get the field from the payload pointer and store the diff function in it.
    if (ai->getParent() == this->original->getEntryBlock()) {
      ...
      builder.createStore(loc, differential, field,
                          StoreOwnershipQualifier::Unqualified);
    } else {
      ...
      builder.createStore(loc, differential, structFieldAddr,
                          StoreOwnershipQualifier::Unqualified);
    }
  ...
  void visitReturnInst(ReturnInst *ri) {
    auto loc = ri->getOperand().getLoc();
//    exitBasicBlock = ri->getParent();
    auto &builder = getBuilder();

    // Get the JVP value corresponding to the original functions's return value.
    auto *origRetInst = cast<ReturnInst>(exitBasicBlock->getTerminator());
    auto origResult = getOpValue(origRetInst->getOperand());
    SmallVector<SILValue, 8> origResults;
    extractAllElements(origResult, builder, origResults);
    ...
    auto *bb0StructVal =
        builder.createLoad(loc, bb0Struct, LoadOwnershipQualifier::Unqualified);
    auto *differentialPartialApply = builder.createPartialApply(
        loc, differentialRef, jvpSubstMap, {bb0StructVal},
        ParameterConvention::Direct_Guaranteed);
